...
h5_distribution = df['impact_factor'].value_counts().sort_index()

# Step 4: Calculate percentiles
percentiles = [0.2, 0.5, 0.75, 0.9]
percentile_values = np.percentile(df['impact_factor'], [p * 100 for p in percentiles])

# Step 5: Visualize the distribution
plt.figure(figsize=(10, 6))
h5_distribution.plot(kind='bar')
plt.title('Distribution of impact_factor Values')
plt.xlabel('impact_factor')
...
for percentile, value in zip(percentiles, percentile_values):
    ...
    plt.text(value, plt.ylim()[1]*0.9, f'{percentile*100:.0f}th percentile: {value:.2f}', 
