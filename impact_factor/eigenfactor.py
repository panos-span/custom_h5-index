...
df = pd.read_csv(
    "reports/get_citation_network.csv",
    header=None,
...
df_aggregated = df.groupby(
    ["cited_issn", "citing_issn", "subject"], as_index=False
).sum()
...
for subject, group in subject_groups:
    citation_matrix = group.pivot(
        index="cited_issn", columns="citing_issn", values="citation_count"
    ).fillna(0)

    # Normalize the matrix to create a transition matrix
    column_sums = citation_matrix.sum(axis=0)
    transition_matrix = citation_matrix.div(column_sums, axis=1).fillna(
        0
    )  # Fill NaN to handle division by zero
    ...
    while delta > tolerance and iteration < max_iterations:
        new_scores = (
            damping_factor * transition_matrix.dot(eigenfactor_scores)
            + random_jump
        )
        delta = np.linalg.norm(
            new_scores - eigenfactor_scores, 1
...
eigenfactor_combined_df.to_sql(
    "eigenfactor_scores", conn, if_exists="replace", index=False
)
