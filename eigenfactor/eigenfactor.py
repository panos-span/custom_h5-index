...
def calculate_eigenfactor_for_subject(group):
    journals = sorted(
        set(group["citing_issn"]).union(set(group["cited_issn"]))
    )
    ...
    while residual > epsilon and iteration < 100:
        new_pi = (
            alpha * H.dot(pi)
            + (alpha * dangling_nodes.dot(pi) + (1 - alpha)) * article_vector
        )
    ...
    eigenfactor_df = pd.DataFrame({"issn": journals, "eigenfactor_score": EF})
