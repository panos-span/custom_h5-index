...
CREATE INDEX IF NOT EXISTS filtered_works_orcid_doi_idx ON rolap.filtered_works_orcid (doi);

-- Index on ORCID for efficient grouping and filtering in h5-index calculations
CREATE INDEX IF NOT EXISTS filtered_works_orcid_orcid_idx ON rolap.filtered_works_orcid (orcid);

CREATE TABLE
    rolap.orcid_h5_filtered AS
WITH ranked_orcid_citations AS (SELECT orcid,
                                       citations_number,
                                       ROW_NUMBER() OVER (
                                           PARTITION BY
                                               orcid
                                           ORDER BY
                                               citations_number DESC
                                           ) AS row_rank
                                FROM rolap.work_citations
                                         INNER JOIN rolap.filtered_works_orcid
                                                    ON rolap.filtered_works_orcid.doi = rolap.work_citations.doi),
     eligible_ranks AS (SELECT orcid,
                               row_rank
                        FROM ranked_orcid_citations
                        WHERE row_rank <= citations_number)
SELECT orcid,
       MAX(row_rank) AS h5_index
FROM eligible_ranks
GROUP BY orcid;