...
CREATE INDEX IF NOT EXISTS rolap.filtered_works_orcid_if3_doi_idx ON filtered_works_orcid_if3 (doi);
-- Index on ORCID for efficient grouping and filtering in h5-index calculations
CREATE INDEX IF NOT EXISTS rolap.filtered_works_orcid_if3_orcid_idx ON filtered_works_orcid_if3 (orcid);

CREATE TABLE rolap.orcid_h5_filtered_if3 AS
WITH ranked_orcid_citations AS (
    SELECT filtered_works_orcid_if3.orcid, citations_number,
           ROW_NUMBER() OVER (
               PARTITION BY orcid ORDER BY citations_number DESC
           ) AS row_rank
    FROM rolap.work_citations
    INNER JOIN rolap.filtered_works_orcid_if3 ON rolap.filtered_works_orcid_if3.doi = rolap.work_citations.doi
),
eligible_ranks AS (
